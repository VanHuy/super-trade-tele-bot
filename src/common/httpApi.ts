...
export const getBalances = async (address: string) => {
  const response: any = await axios.get(
    `https://api.ethplorer.io/getAddressInfo/${address}?apiKey=${process.env.ETHER_API_KEY}`,
    {},
  );
  ...
  let tokenBalances = TOKEN_INFO.replace('<TOKEN-NAME>', 'ETH')
    .replace('<AMOUNT>', etherBalanceInEther)
    .replace('<SYMBOL>', 'ETH');
  ...
  const length = tokens.length > 50 ? 50 : tokens.length;
  for (let index = 0; index < length; index++) {
    ...
    tokenBalances += TOKEN_INFO.replace('<TOKEN-NAME>', tokenSymbol).replace(
      '<AMOUNT>',
      tokenBalanceInTokenUnits.toString(),
    );
  }
  return tokenBalances;
};
...
export const getTokenInfo = async (address: string) => {
  const response = await axios.get(
    `https://api.etherscan.io/api?module=token&action=tokeninfo&contractaddress=${address}&apikey=${process.env.ETHER_API_KEY}`,
  );
  // Get the data from the response
  const data = response.data;
...
export const getTokenInfoEtherplorer = async (address: string) => {
  const response = await axios.get(
    `https://api.ethplorer.io/getTokenInfo/${address}?apiKey=${process.env.ETHER_API_KEY}`,
  );
  // Get the data from the response
